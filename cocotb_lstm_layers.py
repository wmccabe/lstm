...
async def wait_output(dut, x):
    ...
    for _ in x:
        ...
        if verbose: print(f"receiving {dut.y_out.value}, {dut.C_out.value}, {i}/{len(x)}")
        i += 1        
...
async def test_lstm(dut):
    ...
    await cocotb.start_soon(wait_output(dut,x)) # block on wait_output before advancing to error checking

    # error checking
    floating_point_error = abs(lstm.floatingPoint(dut.y_out.value, lstm.precision) - pylstms.layer[dut.LAYERS.value - 1].h_prev)
    floating_point_error_state = abs(lstm.floatingPoint(dut.C_out.value, lstm.precision) - pylstms.layer[dut.LAYERS.value - 1].C_prev)
    if verbose:
        print("--------------------")
        print(x)
