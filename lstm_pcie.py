...
class LAYERED_LSTM_HARDWARE(LAYERED_LSTM):
    def write_config(self, fd):
        for lyr in range(self.layers):
            for gate in range(lstm.gates):
                write_addr, write_data = self.weight_x_address[lyr][gate], self.layer[lyr].fixed_Wx[gate]
                os.pwrite(fd, write_data.to_bytes(8, 'little'), write_addr)
                write_addr, write_data = self.weight_h_address[lyr][gate], self.layer[lyr].fixed_Wh[gate]
                os.pwrite(fd, write_data.to_bytes(8, 'little'), write_addr)
                write_addr, write_data = self.bias_x_address[lyr][gate], self.layer[lyr].fixed_bx[gate]
                os.pwrite(fd, write_data.to_bytes(8, 'little'), write_addr)
                write_addr, write_data = self.bias_h_address[lyr][gate], self.layer[lyr].fixed_bh[gate]
                os.pwrite(fd, write_data.to_bytes(8, 'little'), write_addr)
            
            write_addr, write_data = self.C_prev_address[lyr], self.layer[lyr].fixed_C_prev
            os.pwrite(fd, write_data.to_bytes(8, 'little'), write_addr)

            write_addr, write_data = self.h_prev_address[lyr], self.layer[lyr].fixed_h_prev
            os.pwrite(fd, write_data.to_bytes(8, 'little'), write_addr)
    
    def read_config(self, fd):
        for lyr in range(self.layers):
            for gate in range(lstm.gates):
                read_addr, expected_data = self.weight_x_address[lyr][gate], self.layer[lyr].fixed_Wx[gate]
                read_data = int.from_bytes(os.pread(fd, 8, read_addr), byteorder='little')
                assert expected_data == read_data


                read_addr, expected_data = self.weight_h_address[lyr][gate], self.layer[lyr].fixed_Wh[gate]
                read_data = int.from_bytes(os.pread(fd, 8, read_addr), byteorder='little')
                assert expected_data == read_data


                read_addr, expected_data = self.bias_x_address[lyr][gate], self.layer[lyr].fixed_bx[gate]
                read_data = int.from_bytes(os.pread(fd, 8, read_addr), byteorder='little')
                assert expected_data == read_data


                read_addr, expected_data = self.bias_h_address[lyr][gate], self.layer[lyr].fixed_bh[gate]
                read_data = int.from_bytes(os.pread(fd, 8, read_addr), byteorder='little')
                assert expected_data == read_data
        
            read_addr, expected_data = self.C_prev_address[lyr], self.layer[lyr].fixed_C_prev
            read_data = int.from_bytes(os.pread(fd, 8, read_addr), byteorder='little')
            assert expected_data == read_data

            read_addr, expected_data = self.h_prev_address[lyr], self.layer[lyr].fixed_h_prev
            read_data = int.from_bytes(os.pread(fd, 8, read_addr), byteorder='little')
            assert expected_data == read_data
    ...
    def process_hw(self, fd, x):
        self.process(x)
        for xi in x:
            write_addr, write_data = self.x_in_address, lstm.fixedPoint(xi, lstm.precision)
            os.pwrite(fd, write_data.to_bytes(8, 'little'), write_addr)
            time.sleep(0.1)

        time.sleep(0.1)
        y_out_hw_fixed = int.from_bytes(os.pread(fd, 8, self.y_out_address), byteorder='little')
